...
function topIntentLinesFromMetrics(metrics: MonitorSignalPayload | null) {
  if (!metrics) {
    return [];
  }
  ...
  return Object.entries(metrics.intent_distribution)
    ...
    .slice(0, 3)
    .map(([intent], index) => `Top${index + 1} ${intent.replaceAll("_", " ")}`);
}
...
export default function HostPage() {
  ...
  const topChatIntentLines =
    splitTopChatIntents(
      ...
      : topIntentLinesFromMetrics(hostLiveMetrics);
